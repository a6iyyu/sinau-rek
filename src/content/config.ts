...
export const collections = {
  "blog": defineCollection({
    type: "content",
    schema: z.object({
      title: z.string().trim().min(1, { message: "Title must be at least 1 character!" }).max(200, { message: "Title must be at most 125 characters!" }).regex(/^[A-Za-z0-9\s]+$/, { message: "Title can only contain letters, numbers, and spaces!" }),
